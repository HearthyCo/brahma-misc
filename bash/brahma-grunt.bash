...
for task in $tasks; do
  if [ -d "$task" ]; then
    cd "$task"
    echo -e "${color_1}$task\$ ${color_2}grunt $@$color_no"
    grunt "$@" | sed "s/^/$(echo -e "$color_3""$task" \> "$color_no")/" &
    cd ..
  fi
done
wait
...
for project in $projects; do
  if [ -d "$project" ]; then
    cd "$project"
    echo -e "${color_1}$project\$ ${color_2}grunt $@$color_no"
    grunt "$@" | sed "s/^/$(echo -e "$color_3""$project" \> "$color_no")/" &
    cd ..
  fi
done
wait